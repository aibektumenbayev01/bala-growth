...
async function generatePdf() {
  try {
    ...
    sortedMeasurements.forEach((measurement) => {
      ...
      pdf.text(formatDate(measurement.date), 24, y);
      pdf.text(`${measurement.height} cm`, 90, y);
      pdf.text(`${measurement.weight} kg`, 140, y);
    ...
    if (chartElement) {
      ...
      if (svgElement) {
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          image.onerror = () =>
            reject(
              new Error("Failed to render WHO chart")
            );
