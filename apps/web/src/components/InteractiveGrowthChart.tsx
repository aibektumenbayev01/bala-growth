...
function ChartPanel({metric,active,ageMonths,reference,value,onSelect}:{metric:GrowthMetric;active:boolean;ageMonths:number;reference:LmsReferencePoint[];value:number|null;onSelect:(value:number)=>void}){
 ...
 function pick(event:React.PointerEvent<HTMLDivElement>){
  ...
  // Recharts reserves the full XAxis height inside the plotting area. The
  // stature axis is above the plot; the weight axis is below it.
  const top=margins.top+(metric==="height"?xAxisHeight:0);
  const bottom=rect.height-margins.bottom-(metric==="weight"?xAxisHeight:0);
  const y=Math.max(top,Math.min(bottom,event.clientY-rect.top));
  const raw=domain[0]+((bottom-y)/(bottom-top))*(domain[1]-domain[0]);
  onSelect(Number((Math.round(raw*10)/10).toFixed(1)));
 }
...
export default function InteractiveGrowthChart(props:Props){const ratio=Math.max(0,Math.min(1,(props.ageMonths-24)/216));const agePosition=`calc(56px + (100% - 102px) * ${ratio})`;return <div className="relative h-[600px] min-h-[480px] overflow-hidden border border-slate-300 bg-white"><div className="absolute left-1/2 top-1 z-20 -translate-x-1/2 text-xs font-semibold tracking-wide">AGE (YEARS)</div><ChartPanel metric="height" active={props.activeMetric==="height"} ageMonths={props.ageMonths} reference={props.reference} value={props.height} onSelect={v=>props.onSelect("height",v)}/><div className="border-t-2 border-slate-500"/><ChartPanel metric="weight" active={props.activeMetric==="weight"} ageMonths={props.ageMonths} reference={props.reference} value={props.weight} onSelect={v=>props.onSelect("weight",v)}/><div className="pointer-events-none absolute bottom-0 -translate-x-1/2 rounded-t bg-white px-1 text-sm font-bold text-blue-600" style={{left:agePosition}}>{(props.ageMonths/12).toFixed(1)}</div></div>}
